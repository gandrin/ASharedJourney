...
func GenerateMap(levelFileName string) World {
	...
	assetDirectory, err := filepath.Abs(path.Join("./", "assets"))
	if err != nil {
		log.Fatal("error getting asset directory")
	}
	filemap := path.Join(assetDirectory, levelFileName+".tmx")
	filetile := path.Join(assetDirectory, tilesPath)
